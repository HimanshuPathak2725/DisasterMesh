...
## Corpus Check
- 58 files · ~36,921 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 915 nodes · 1791 edges · 52 communities (43 shown, 9 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 373 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c1d7dd98`
...
Cohesion: 0.12
Nodes (20): _proto_to_document(), Store a ProtoIncident and its pre-computed embedding in Qdrant., Semantic similarity search using LangChain interface.          Returns list of (, Return the total number of points in the collection., Persist a :class:`~app.schemas.VerifiedIncident` back into the Qdrant         co, Convert a ProtoIncident to a LangChain Document.      page_content  = the text u, LangChain-based vector store backed by Qdrant.      Wraps QdrantVectorStore for, VectorStore (+12 more)
...
Cohesion: 0.14
Nodes (10): init_vector_store(), QdrantClient, Vector Store — Phase 2.  Uses LangChain's QdrantVectorStore wrapper so both the, Create the Qdrant collection if it doesn't exist, then bind         the LangChai, Convert a UUID string to an integer suitable as a Qdrant point ID., Initialise the VectorStore singleton and ensure the Qdrant collection exists., _uuid_to_int(), memory_vector_store() (+2 more)
...
- **Why does `VerifiedIncident` connect `VerificationAgent` to `🏗️ Architecture`, `📦 Data schema`, `VerifiedIncident`, `test_ingest.py`, `incidents.py`, `SatellitePolygonInput`, `SocialPostInput`, `AsyncSession`, `CommunicationAgent`, `test_schemas.py`, `.verify`, `.upsert`, `TestNotificationMockMode`, `_record_to_schema`, `TestStateMachine`, `main.py`, `get_qdrant_client_sync`, `.assess`, `SourceType`?**
  _High betweenness centrality (0.128) - this node is a cross-community bridge._
- **Why does `NeedsProfile` connect `CommunicationAgent` to `🏗️ Architecture`, `TestNotificationMockMode`, `📦 Data schema`, `VerifiedIncident`, `TestStateMachine`, `VerificationAgent`, `get_qdrant_client_sync`, `test_ingest.py`, `CitizenReportInput`, `incidents.py`, `SourceType`, `SatellitePolygonInput`, `AsyncSession`, `CommunicationAgent`, `test_schemas.py`, `.verify`, `.upsert`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `VectorStore` connect `SocialPostInput` to `vector_store.py`, `init_vector_store`, `VerificationAgent`, `incidents.py`, `SatellitePolygonInput`, `test_verification_integration.py`?**
