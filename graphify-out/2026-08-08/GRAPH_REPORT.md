...
## Summary
- 723 nodes · 1372 edges · 41 communities (30 shown, 11 thin omitted)
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 269 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `592884f6`
...
- `DispatchState` --uses--> `ResourceAgent`  [INFERRED]
  backend/app/agents/orchestrator.py → backend/app/agents/resource.py
- `DispatchState` --uses--> `DispatchRecord`  [INFERRED]
  backend/app/agents/orchestrator.py → backend/app/models.py
- `DispatchState` --uses--> `DispatchStatus`  [INFERRED]
  backend/app/agents/orchestrator.py → backend/app/schemas.py
- `DispatchState` --uses--> `NeedsProfile`  [INFERRED]
  backend/app/agents/orchestrator.py → backend/app/schemas.py
...
Cohesion: 0.06
Nodes (66): get_resource_agent(), _haversine_m(), AsyncSession, Resource Agent — Agent 4.  Responsibilities:   - Maintain live responder registr, Return all responders, optionally filtered by status., Fetch a single responder by id; returns None if not found., Update a responder's GPS position.          Returns the updated Responder, or No, Update a responder's operational status.          When transitioning back to 'av (+58 more)

### Community 7 - "schemas.py"
Cohesion: 0.08
Nodes (33): AuditLog, Base, DispatchRecord, SQLAlchemy ORM models for DisasterMesh.  Tables ------ raw_ingestion_records  —, Immutable record of each assignment made by the Orchestrator Agent.      Created, Persists the raw payload **and** the normalised ProtoIncident produced     by th, Immutable append-only audit trail — captures who/what/when for every     signifi, RawIngestionRecord (+25 more)
...
Cohesion: 0.06
Nodes (57): _build_dispatch_graph(), _cap_score(), commit_assignments(), DispatchState, _eta_seconds(), fetch_responders(), get_orchestrator_agent(), _haversine_m() (+49 more)
...
Cohesion: 0.17
Nodes (16): get_embedding_service(), Return the shared EmbeddingService singleton., ProtoIncident, Normalized incident before verification and deduplication., memory_vector_store(), Integration tests for VectorStore with Qdrant — Phase 2.  Uses an in-memory Qdra, Create a fresh in-memory VectorStore for each test., test_search_nearby_geo_radius() (+8 more)
...
### Community 31 - ".upsert"
Cohesion: 0.29
Nodes (9): async_sessionmaker, get_db(), _get_engine(), _get_session_factory(), init_db(), AsyncSession, Database and service client factories.  Qdrant:      local file mode by default, Create all ORM tables on startup (idempotent).      Called once from the FastAPI (+1 more)

### Community 36 - "get_redis_client"
Cohesion: 0.24
Nodes (7): get_settings(), DisasterMesh backend — application settings.  Loaded from environment variables, Return cached settings singleton., Settings, health(), Returns 200 when the API is up. Used by CI and load balancers., BaseSettings

### Community 37 - "init_vector_store"
Cohesion: 0.09
Nodes (22): Any, _haversine_m(), init_vector_store(), Vector Store — Phase 2.  Uses LangChain's QdrantVectorStore wrapper so both the, Create the Qdrant collection if it doesn't exist, then bind         the LangChai, Find ProtoIncident payloads (and optional vectors) within geo radius + optional, Fetch a payload by proto_id., Return the payload dict for the *verified* point with the given cluster_id. (+14 more)

### Community 40 - "main.py"
Cohesion: 0.67
Nodes (3): lifespan(), DisasterMesh FastAPI application entrypoint.  Run locally:     cd backend     uv, FastAPI
...
- **Why does `VerifiedIncident` connect `VerificationAgent` to `init_vector_store`, `VerifiedIncident`, `📦 Data schema`, `CommunicationAgent`, `incidents.py`, `SatellitePolygonInput`, `AsyncSession`, `test_schemas.py`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `ProtoIncident` connect `SocialPostInput` to `init_vector_store`, `📦 Data schema`, `schemas.py`, `CommunicationAgent`, `embeddings.py`, `VerifiedIncident`, `incidents.py`, `SatellitePolygonInput`, `AsyncSession`, `test_verification_integration.py`?**
  _High betweenness centrality (0.091) - this node is a cross-community bridge._
- **Why does `VectorStore` connect `init_vector_store` to `CommunicationAgent`, `VerificationAgent`, `embeddings.py`, `incidents.py`, `SatellitePolygonInput`, `SocialPostInput`, `test_verification_integration.py`?**
